...
public class TicTacToeController {
    ...
    public void processButtonClick(int row, int col) {
        ...
        screen.setMessage("It is " + playerChar[(currPlayer + 1) % numPlayers] + "'s turn. ");
        ...
        if (curGame.checkSpace(new BoardPosition(row, col))) {
            ...
            screen.setMessage("That space is unavailable! Player " + playerChar[(currPlayer) % numPlayers] + ", please try again.");
            return;
        }

        // Check for win conditions
        if (curGame.checkForWinner(new BoardPosition(row, col))) {
            screen.setMessage(playerChar[(currPlayer) % numPlayers] + " has won! Click any button to play again.");
            end = true;
        }
        // Check for draw
